...
function summaryHtml(){
  ...
  const rows=standings.map(({p,territories,troops,stats},i)=>`<tr class="${p.id===state.winner?'end-winner-row':''}"><td><span class="end-rank">${i+1}</span><i class="end-player-dot" style="--pc:${p.color}"></i>${escape(p.name)}${p.id===state.winner?' <b>Ganador</b>':''}</td><td>${territories}</td><td>${troops}</td>${campaign?.complete?`<td>${stats.conquests}</td><td>${stats.lost}</td>`:''}</tr>`).join('');
  ...
  return `<div class="end-hero" style="background-image:linear-gradient(90deg,#061523f0,#06152377),url('./map-${map.id}.webp')"><span>MAPA DOMINADO</span><strong>${escape(map.name)}</strong><small>${state.rulesMode==='terrain'?'Modo terreno':'Modo clásico'} · ${state.turn} ${state.turn===1?'ronda':'rondas'} · ${state.players.length} comandantes</small></div>
    ...
    <h3>Balance final</h3><div class="end-table-wrap"><table class="end-table"><thead><tr><th>Comandante</th><th>Territorios</th><th>Tropas</th>${campaign?.complete?'<th>Conquistas</th><th>Bajas</th>':''}</tr></thead><tbody>${rows}</tbody></table></div>
