import {REGIONS,MAPS,TERRAINS,UNIT_TYPES,getMap,getTerritories,createGame,ownedIds,enemiesOf,placeTroops,attackRound,blitz,fortify,setPhase,endTurn,aiTurn,validateState,canPlayerAttack,tradeCards,territoryProduction,productionTotal,buyReinforcements,upgradeGame} from './engine.mjs';
...
let state=null,pendingAiState=null,difficulty='normal',selectedFrom=null,selectedTo=null,selectedDice=3,selectedUnit='infantry',toastTimer=null,aiBusy=false,rolling=false,routesAll=false,hoverId=null,aiResolve=null,defenseResolve=null;
...
function updateHover(id,e){if(!state)return;const t=tById(id),d=state.territories[id],p=state.players[d.owner],terrain=TERRAINS[t.terrain],unit=UNIT_TYPES[d.unitType];els.mapTooltip.innerHTML=`<b>${t.name}</b><div class="tip-row"><span>Propietario</span><strong style="color:${p.color}">${p.name}</strong></div><div class="tip-row"><span>Tropas</span><strong>${d.troops}</strong></div><div class="tip-row"><span>Producción</span><strong>+$${territoryProduction(state,id)}</strong></div>${state.rulesMode==='terrain'?`<div class="tip-row"><span>Terreno</span><strong>${terrain.icon} ${terrain.name}</strong></div><div class="tip-row"><span>Unidad</span><strong>${unit.icon} ${unit.name}</strong></div>`:''}<div class="tip-adj">Conecta con ${t.n.length} territorios</div>`;els.mapTooltip.classList.remove('hidden');positionTooltip(e);renderMap()}
...
function renderEconomy(){const p=state.players[state.current],income=productionTotal(state,p.id),canBuy=p.human&&state.phase==='reinforce'&&p.money>=10;els.economyBox.innerHTML=`<div class="economy-head"><span>TESORO</span><strong>$${p.money}</strong></div><small>Producción actual: +$${income} al inicio del turno.</small>${p.human&&state.phase==='reinforce'?`<button id="buyTroopsBtn" class="secondary-btn" ${canBuy?'':'disabled'}>+3 refuerzos · $10</button>`:''}`;const button=$('#buyTroopsBtn');if(button)button.onclick=()=>{if(buyReinforcements(state)){render();showToast('Compraste 3 refuerzos por $10')}}}
...
function summaryHtml(){
  ...
  const standings=state.players.map(p=>({p,territories:ownedIds(state,p.id).length,troops:ownedIds(state,p.id).reduce((sum,id)=>sum+state.territories[id].troops,0),stats:campaign?.complete?campaign.players[p.id]:null})).sort((a,b)=>b.territories-a.territories||b.troops-a.troops);
  const rows=standings.map(({p,territories,troops,stats},i)=>`<tr class="${p.id===state.winner?'end-winner-row':''}"><td><span class="end-rank">${i+1}</span><i class="end-player-dot" style="--pc:${p.color}"></i>${escape(p.name)}${p.id===state.winner?' <b>Ganador</b>':''}</td><td>${territories}</td><td>${troops}</td><td>$${p.money}</td>${campaign?.complete?`<td>${stats.conquests}</td><td>${stats.lost}</td>`:''}</tr>`).join('');
  ...
  return `<div class="end-hero" style="background-image:linear-gradient(90deg,#061523f0,#06152377),url('./map-${map.id}.webp')"><span>MAPA DOMINADO</span><strong>${escape(map.name)}</strong><small>${state.rulesMode==='terrain'?'Modo terreno':'Modo clásico'} · ${state.turn} ${state.turn===1?'ronda':'rondas'} · ${state.players.length} comandantes</small></div>
    <div class="end-kpis"><div><strong>${map.territories.length}</strong><span>territorios</span></div><div><strong>${state.turn}</strong><span>rondas</span></div><div><strong>${standings[0].troops}</strong><span>tropas vencedoras</span></div>${campaign?.complete?`<div><strong>${campaign.players.reduce((n,p)=>n+p.rolls,0)}</strong><span>tiradas</span></div>`:''}</div>
    <h3>Balance final</h3><div class="end-table-wrap"><table class="end-table"><thead><tr><th>Comandante</th><th>Territorios</th><th>Tropas</th><th>Tesoro</th>${campaign?.complete?'<th>Conquistas</th><th>Bajas</th>':''}</tr></thead><tbody>${rows}</tbody></table></div>
    ${campaign?.complete?`<h3>Actividad de la campaña</h3><div class="end-activity">${standings.map(({p,stats})=>`<div><i class="end-player-dot" style="--pc:${p.color}"></i><strong>${escape(p.name)}</strong><span>${stats.rolls} tiradas · ${stats.defeated} rivales eliminados · ${stats.cards} cartas robadas · ${stats.trades} canjes</span></div>`).join('')}</div><details class="end-history"><summary>Historial completo de conquistas <span>${campaign.conquests.length}</span></summary><ol>${history||'<li>No hubo conquistas registradas.</li>'}</ol></details>`:`<p class="end-legacy">Esta partida comenzó antes del registro de estadísticas. El balance final muestra los datos disponibles del mapa.</p>`}`;
}
function showEndSummary(){
  ...
  $('#endTitle').textContent='Mapa conquistado';
  $('#endSubtitle').textContent=won?`Conquistaste ${getMap(state).name}. Todos los territorios están bajo tu control.`:`${winner.name} conquistó ${getMap(state).name}. Tu ejército ha sido eliminado.`;
