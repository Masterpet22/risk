...
export function aiTurn(state,pid=state.current,difficulty='normal'){
  ...
  while(moves++<limit&&state.winner===null){
    ...
    if(result.ok){
      ...
      report.battles.push({from:terr(state,best.from).name,to:terr(state,best.to).name,rounds:result.rounds.length,attackerLosses:al,defenderLosses:dl,conquered:result.conquered,eliminated:last.eliminated,bonus:last.bonus});
